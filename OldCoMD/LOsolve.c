...
void plotFields(FILE *gp) {
    ...
    //fprintf(gp, "plot '-' u 1:2 ti 'A' w lp\n\n");
...
void writeFields(FILE *fp) {
    int i;
    fprintf(fp, "%3d: ", loop_count);
    for (i = 0; i < L; i++) {
        fprintf(fp, "%f ", A[i]);
    }
    fprintf(fp, "\n");
...
int main(int argc, char **argv) {
    ...
    FILE *fStress = fopen ("stress.txt", "w");
    if (fStress == NULL) {
        printf("Error opening stress file < %s >!\n", "stress.txt");
        exit(0);
    }
